...
abstract contract BaseCustomCurve is BaseCustomAccounting {
        ...
        returns (bytes4, BeforeSwapDelta returnDelta, uint24)
    {
        ...
        if (exactInput) {
            ...
            // 1. Mint ERC-6909 claims for the specified input amount
            specified.take(poolManager, address(this), specifiedAmount, true);
            // 2. Burn ERC-6909 claims for the calculated output amount
            unspecified.settle(poolManager, address(this), unspecifiedAmount, true);

            returnDelta = toBeforeSwapDelta(specifiedAmount.toInt128(), -unspecifiedAmount.toInt128());
        } else {
            // For exact output swaps:
            // 1. Mint ERC-6909 claims for the calculated input amount
            unspecified.take(poolManager, address(this), unspecifiedAmount, true);
            // 2. Burn ERC-6909 claims for the specified output amount
     ...
     *
     * @param key The pool key for the pool to modify liquidity in.
        ...
        returns (BalanceDelta callerDelta, BalanceDelta feesAccrued)
    {
        ...
        (callerDelta, feesAccrued) = abi.decode(
            poolManager.unlock(abi.encode(CallbackDataCustom(key, msg.sender, amount0, amount1))),
