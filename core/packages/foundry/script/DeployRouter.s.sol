...
import { FixedPriceLicenseHook } from "../contracts/hook/fixed/FixedPriceLicenseHook.sol";
import { DynamicPriceLicenseHook } from "../contracts/hook/dynamic/DynamicPriceLicenseHook.sol";
import { LicenseSwapRouter } from "../contracts/LicenseSwapRouter.sol";
...
/**
 * @notice Deploy script for LicenseSwapRouter and all dependencies
 * @dev Deploys: MockNumeraire, IPERC721, hooks, LicenseSwapRouter, CampaignManager
 *
 * Run with mainnet fork:
 *   forge script script/DeployRouter.s.sol --broadcast --rpc-url http://localhost:8545 --private-key <key>
 *
 * Start anvil with mainnet fork first:
 *   anvil --fork-url https://eth-mainnet.g.alchemy.com/v2/<API_KEY>
 */
contract DeployRouter is ScaffoldETHDeploy {
    ...
    FixedPriceLicenseHook public fixedPriceLicenseHook;
    DynamicPriceLicenseHook public dynamicPriceLicenseHook;
    LicenseSwapRouter public router;
    ...
    function run(address authCaptureEscrow, address tokenCollector) external ScaffoldEthDeployerRunner {
        console.log("Deploying LicenseSwapRouter system...");
        ...
        // Step 4: Deploy hooks at correct address (needs patentNFT)
        _deployFixedPriceLicenseHook();
        _deployDynamicPriceLicenseHook();

        // Step 5: Deploy router
        _deployRouter();

        // Step 6: Deploy campaign manager
        _deployCampaignManager(authCaptureEscrow, tokenCollector);
    ...
    function _deployMockTokens() internal {
        ...
        // Deploy numeraire (mock USDC); use script/MintNumeraire.s.sol to mint to recipients
    ...
    function _deployFixedPriceLicenseHook() internal {
        ...
        fixedPriceLicenseHook = new FixedPriceLicenseHook{ salt: salt }(poolManager, deployer, IERC721(address(patentNFT)));
        require(address(fixedPriceLicenseHook) == hookAddress, "Hook address mismatch");

        console.log("FixedPriceLicenseHook deployed at:", address(fixedPriceLicenseHook));
        deployments.push(Deployment("FixedPriceLicenseHook", address(fixedPriceLicenseHook)));
    }

    function _deployDynamicPriceLicenseHook() internal {
        console.log("Deploying DynamicPriceLicenseHook...");

        // Hook flags from DynamicPriceLicenseHook.getHookPermissions()
        uint160 flags = uint160(
        ...
        bytes memory constructorArgs = abi.encode(address(poolManager), deployer, IERC721(address(patentNFT)));
        (address hookAddress, bytes32 salt) =
            HookMiner.find(CREATE2_DEPLOYER, flags, type(DynamicPriceLicenseHook).creationCode, constructorArgs);

        console.log("Found hook address:", hookAddress);
        console.log("Using salt:", vm.toString(salt));

        dynamicPriceLicenseHook =
            new DynamicPriceLicenseHook{ salt: salt }(poolManager, deployer, IERC721(address(patentNFT)));
        require(address(dynamicPriceLicenseHook) == hookAddress, "Hook address mismatch");

        console.log("DynamicPriceLicenseHook deployed at:", address(dynamicPriceLicenseHook));
        deployments.push(Deployment("DynamicPriceLicenseHook", address(dynamicPriceLicenseHook)));
    }

    function _deployRouter() internal {
        console.log("Deploying LicenseSwapRouter...");
        address authorizedRelayer = vm.envAddress("ADMIN_APP_ADDRESS");
        router = new LicenseSwapRouter(poolManager, permit2, authorizedRelayer);
        console.log("LicenseSwapRouter deployed at:", address(router));
        deployments.push(Deployment("LicenseSwapRouter", address(router)));
    }

    function _deployCampaignManager(address authCaptureEscrow, address tokenCollector) internal {
        console.log("Deploying CampaignManager...");

        address adminAppAddress = vm.envAddress("ADMIN_APP_ADDRESS");
        ...
        fixedPriceLicenseHook.transferOwnership(address(campaignManager));
        dynamicPriceLicenseHook.transferOwnership(address(campaignManager));
    ...
    function _logSummary() internal view {
        ...
        console.log("FixedPriceLicenseHook: ", address(fixedPriceLicenseHook));
        console.log("DynamicPriceLicenseHook:", address(dynamicPriceLicenseHook));
        console.log("LicenseSwapRouter:     ", address(router));
