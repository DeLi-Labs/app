...
import { BadRequestError, NotFoundError } from "~~/utils/scaffold-eth/errors"
...
export class LocalDatabaseStorageGateway implements IStorageGateway {
  ...
  /** Resolve env/relative paths against cwd so uploads and reads hit the same file across dev/build/deploy. */
  private static resolveStorageFilePath(explicit?: string): string {
    const raw = explicit ?? process.env.LOCAL_STORAGE_PATH ?? join(process.cwd(), "local-storage.json");
    return isAbsolute(raw) ? raw : resolve(process.cwd(), raw);
  }

  constructor(storageFilePath?: string) {
    this.storageFilePath = LocalDatabaseStorageGateway.resolveStorageFilePath(storageFilePath);
  ...
  async retrieve(uri: string): Promise<RetrieveResult> {
    if (!uri.startsWith("local://")) {
      throw new BadRequestError(`Invalid local storage URI: ${uri}`);
    }
    ...
    if (!entry) {
      console.warn(
        `[LocalDatabaseStorageGateway] Missing ${uri} (${this.storage.size} keys loaded). ` +
          `File: ${this.storageFilePath}. Indexer metadata points at a blob that is not in this file — ` +
          `use the same LOCAL_STORAGE_PATH as upload, restore the file, or use IPFS in production.`,
      );
      throw new NotFoundError(`Content not found: ${uri}`);
    }
