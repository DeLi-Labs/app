...
/** Apply `scaffold.config` RPC overrides to a viem chain (used by wagmi, Privy, and direct clients). */
export function withConfiguredRpcUrls(chain: chains.Chain): chains.Chain {
  const rpcOverride = (scaffoldConfig.rpcOverrides as Record<number, string> | undefined)?.[chain.id];
...
export function getRpcHttpUrl(chainId: number): string {
  ...
  return withConfiguredRpcUrls(chain).rpcUrls.default.http[0] ?? "";
}
