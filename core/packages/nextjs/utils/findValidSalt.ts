 ...
 * @param licenseType - License type (0 = DYNAMIC, 1 = FIXED; matches CampaignManager LicenseType enum)
 ...
 * @param chainId - Chain ID to use (caller must pass the app-selected network, e.g. from useTargetNetwork)
...
export async function findValidSalt(
  ...
  if (!campaignManagerContract) {
    throw new Error(`CampaignManager contract not found on chain ${chainId}`);
  }
