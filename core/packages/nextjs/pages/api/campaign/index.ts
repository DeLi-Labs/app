...
import { parseCampaignFormData } from "~~/utils/apiUtils";
...
class CampaignHandler {
  ...
  async uploadCampaignMetadata(@Req() req: IncomingMessage) {
    ...
    const campaignProvider = new CampaignProvider(parseResult.chainId);

    try {
      const metadataUri = await campaignProvider.uploadCampaignMetadata(parseResult.data);
